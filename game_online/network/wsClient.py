...
class GameWebSocketClient:
    def __init__(self, server_url: str, player_uuid: str,
                 on_game_state: Callable[[List[Dict[str, Any]]], None],
                 on_connected: Optional[Callable[[], None]] = None,   # 新增：连接成功回调
        ...
        self.on_connected = on_connected   # 连接成功后调用（无参数）
        ...
        while self._running:
            try:
                ...
                async with websockets.connect(uri, ping_interval=20, ping_timeout=10, close_timeout=5) as ws:
                    self._ws = ws
                    # 连接成功，触发回调（由上层发送 join 等初始化消息）
                    if self.on_connected:
                        # on_connected 是同步函数，在异步环境中直接调用即可
                        self.on_connected()
                    # 接收消息循环
    ...
    async def _handle_message(self, raw_msg):
        try:
            ...
            if msg_type == "game_state":
                # 服务器消息结构：{"type": "game_state", "snapshots": {"Players": [...]}}
